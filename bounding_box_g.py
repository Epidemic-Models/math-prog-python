...
class BoundingBoxG(Rectangle):
    ...
    def __str__(self) -> str:
        return "BoundingBoxG: centre=" + str(self.centre) + \
               ", width=" + str(self.width) + ", height=" + str(self.height)

    def convert(self) -> BoundingBox:
        return BoundingBox(width=self.width, height=self.height, left_upper=self.left_upper)
    ...
    def get_union(self, other: BoundingBoxG) -> BoundingBoxG:
        ...
        shift = Point(p=(union_height / 2,  union_width / 2))
        union_centre = union_left_upper + shift
        return BoundingBoxG(centre=union_centre, width=union_width, height=union_height)

    def get_intersection(self, other: BoundingBoxG) -> BoundingBoxG:
        ...
        if intersection_height > 0 and intersection_width > 0:
            shift = Point(p=(intersection_height / 2, intersection_width / 2))
            intersection_centre = intersection_left_upper + shift
            intersection = BoundingBoxG(centre=intersection_centre,
...
def main() -> None:
    bounding_box_1 = BoundingBoxG(centre=Point(p=(3, 5)), width=3, height=3)
    print(bounding_box_1)
    bounding_box_2 = BoundingBoxG(centre=Point(p=(1, 2)), width=3, height=3)
